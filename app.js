...
class App extends Homey.App {
	...
	async attachScheduler(params) {
			...
			capabilitySchedulers[params.seconds] = this.homey.setInterval(function () {
				for (const device in capabilityTimers[params.seconds]) {
					...
					try {
						...
						self.error(`Failed scheduled logging for device with id ` + id + `. Device cleanup needed!`);
		...
		try {
			...
			this.error(`Someting went wrong attaching the scheduler: "` + JSON.stringify(params));
		}
		var DevCap = params.id.concat("-", params.capability);
		if (!(DevCap in capabilityListeners)) {
			this.log('Also attaching a listener to update the values');
			this.getDeviceById(params.id).then((device) => {
				this.log(`Attaching a listener on the ` + params.capability + ` capability of device "` + device.name + `".`);
				try {
					var DevCap = params.id.concat("-", params.capability);
	...
	async deviceCleanup(sqlDevices) {
		this.log(`Cleaning non-existing devices`);
		let allDevices = await this.getDevices();
		var deletedDevices = 0;
		for (let id in sqlDevices) {
			if (!(id in allDevices)) {
				delete sqlDevices[id];
				this.log(`Deleting device formerly known with id ` + id);
				deletedDevices = deletedDevices + 1;
			}
		};
		if (deletedDevices != 0) {
			this.log(`Found ` + deletedDevices + ` non existing devices. It is strongly advised te restart this app!`);
			this.homey.settings.set('sqlDevices', sqlDevices);
		} else {
			this.log(`No non-existing devices found, system seems healthy!`);
		}
	}

	async testSQLConnection(sqlConfig) {
		// var sqlConfigWithMaskedPassword = sqlConfig
		// sqlConfigWithMaskedPassword.password = "**********";
		// this.log(`SQL Connection test started with the following settings: ` + JSON.stringify(sqlConfigWithMaskedPassword));
		...
		var config = 'Server=' + sqlConfig.server + ';Database=' + sqlConfig.database + ';User Id=' + sqlConfig.user + ';Password=' + sqlConfig.password + ';Encrypt=' + sqlConfig.sqlsvr_encrypt + ';TrustServerCertificate=' + sqlConfig.sqlsvr_trustcertificate;
		sql.close();
	...
	async initSQLdb() {
		var sqlSettings = this.homey.settings.get('sqlSettings');
		// var sqlSettingsWithMaskedPassword = sqlSettings
		// sqlSettingsWithMaskedPassword.password = "**********";
		// this.log(`Connection to SQL Server with the following settings: ` + JSON.stringify(sqlSettingsWithMaskedPassword));
		...
		sqlPool = new sql.ConnectionPool({
			...
			options: {
				...
				encrypt: sqlSettings.sqlsvr_encrypt,
				trustServerCertificate: sqlSettings.sqlsvr_trustcertificate,
				stream: true
			},
			pool: {
		...
		sqlPool.on("error", (err) => {
			this.error(`sqlPool.on(error): ` + err);
		});
		...
		try {
			...
			legacyMode = true;
			try {
				this.log(`Checking for a LogType column`);
				await sqlPool.request().query("IF COL_LENGTH('[Homey2InsightsData].[dbo].[InsightsData]', 'LogType') IS NULL BEGIN SELECT 'true' as legacyMode END ELSE BEGIN SELECT 'false' as legacyMode END").then(result => {
					if (!result.rowsAffected[0]) {
						this.error("sqlPool.request() anomaly: affected rows < 1. (" + result + ")");
						this.log(`Assuming legacyMode`);
					} else {
						legacyMode = (result.recordset[0].legacyMode === 'true');
						this.log('legacyMode is : ' + legacyMode);
					}
				}).catch(err => {
					this.error(`IF COL_LENGTH('[Homey2InsightsData].[dbo].[InsightsData]', 'LogType') IS NULL BEGIN SELECT 'true' as legacyMode END ELSE BEGIN SELECT 'false' as legacyMode END`);
					this.error(`sqlPool.request() error: `, err);
					this.log(`Assuming legacyMode`);
				});
			}
			catch (err) {
				this.error(`Error: Unable to alter database!`);
				this.error(`IF COL_LENGTH('[Homey2InsightsData].[dbo].[InsightsData]', 'LogType') IS NULL BEGIN ALTER TABLE [Homey2InsightsData].[dbo].[InsightsData] ADD [LogType] INT NULL END`);
				this.error(err);
			}
		} catch (err) {
	...
	async logCapabilityData(device, capabilityName, capabilityData, logType) {
		if (capabilityData != null) {
			if (legacyMode) {
				try {
					sqlPool.request().query("Insert into InsightsData (TimeStampSQL, TimeStampEvent, DeviceUri, DeviceName, CapabilityName, CapabilityData, TimedData) VALUES (GETDATE(), '" + luxon.DateTime.now().setZone(this.homey.clock.getTimezone()).toFormat('yyyy-MM-dd HH:mm:ss.SSS') + "', '" + device.id + "', '" + device.name + "', '" + capabilityName + "', '" + capabilityData + "', '" + Boolean(logType) + "')").then(result => {
						if (!result.rowsAffected[0]) {
							this.error("sqlPool.request() anomaly: affected rows < 1. (" + result + ")");
							this.error(`Insert into InsightsData (TimeStampSQL, TimeStampEvent, DeviceUri, DeviceName, CapabilityName, CapabilityData, TimedData) VALUES (GETDATE(), '` + luxon.DateTime.now().setZone(this.homey.clock.getTimezone()).toFormat('yyyy-MM-dd HH:mm:ss.SSS') + `', '` + device.id + `', '` + device.name + `', '` + capabilityName + `', '` + capabilityData + `', '` + Boolean(logType) + `')`);
						}
					}).catch(err => {
						this.error("sqlPool.request() error: ", err);
					});
					...
					this.error(`Error: function logCapabilityData(` + device.name + `,` + capabilityName + `,` + capabilityData + `,` + Boolean(logType) + `,` + luxon.DateTime.now().setZone(this.homey.clock.getTimezone()).toFormat('yyyy-MM-dd HH:mm:ss.SSS') + `)`);
					this.error(err);
				}
			} else {
				try {
					if (logType % 2 == 0) {
						sqlPool.request().query("Insert into InsightsData (TimeStampSQL, TimeStampEvent, DeviceUri, DeviceName, CapabilityName, CapabilityData, LogType) VALUES (GETDATE(), '" + luxon.DateTime.now().setZone(this.homey.clock.getTimezone()).toFormat('yyyy-MM-dd HH:mm:ss.SSS') + "', '" + device.id + "', '" + device.name + "', '" + capabilityName + "', '" + capabilityData + "', " + logType + ")").then(result => {
							if (!result.rowsAffected[0]) {
								this.error("sqlPool.request() anomaly: affected rows < 1. (" + result + ")");
								this.error(`Insert into InsightsData (TimeStampSQL, TimeStampEvent, DeviceUri, DeviceName, CapabilityName, CapabilityData, LogType) VALUES (GETDATE(), '` + luxon.DateTime.now().setZone(this.homey.clock.getTimezone()).toFormat('yyyy-MM-dd HH:mm:ss.SSS') + `', '` + device.id + `', '` + device.name + `', '` + capabilityName + `', '` + capabilityData + `', ` + logType + `)`);
							}
						}).catch(err => {
							this.error("sqlPool.request() error: ", err);
						});
					} else {
						sqlPool.request().query("Update top(1) InsightsData Set TimeStampSQL = GETDATE(), TimeStampEvent = '" + luxon.DateTime.now().setZone(this.homey.clock.getTimezone()).toFormat('yyyy-MM-dd HH:mm:ss.SSS') + "', CapabilityData = '" + capabilityData + "', LogType = '" + logType + "' Where DeviceUri = '" + device.id + "' and CapabilityName = '" + capabilityName + "'").then(result => {
							if (!result.rowsAffected[0]) {
								this.error("sqlPool.request() anomaly: affected rows < 1. (" + result + ")");
								this.error(`Update top(1) InsightsData Set TimeStampSQL = GETDATE(), TimeStampEvent = '` + luxon.DateTime.now().setZone(this.homey.clock.getTimezone()).toFormat('yyyy-MM-dd HH:mm:ss.SSS') + `', CapabilityData = '` + capabilityData + `', LogType = '` + logType + `' Where DeviceUri = '` + device.id + `' and CapabilityName = '` + capabilityName + `')`);
							}
						}).catch(err => {
							this.error(`sqlPool.request() error: `, err);
						});
					...
					this.error(`Error: function logCapabilityData(` + device.name + `,` + capabilityName + `,` + capabilityData + `,` + logType + `,` + luxon.DateTime.now().setZone(this.homey.clock.getTimezone()).toFormat('yyyy-MM-dd HH:mm:ss.SSS') + `)`);
					this.error(err);
				}
			}
		} else {
			this.log('capabilityData is null?');
			this.log(device.name);
			this.log(typeof capabilityData, `:`, capabilityData);
			this.log(capabilityName, `:`, logType);
		}
	}
